...
pub(crate) fn execute<'a>(
    ...
    let mut chip_candidates = vec![i2c_address, I2C_ADDRESS_DDC_CI, I2C_ADDRESS_DDC_CI_MDCP29XX];
    chip_candidates.sort_unstable();
    chip_candidates.dedup();
    if let Some(primary_index) = chip_candidates.iter().position(|chip| *chip == i2c_address) {
        chip_candidates.rotate_left(primary_index);
    }

    let data_addr_candidates = [SUB_ADDRESS_DDC_CI as u16, 0x50];
    let read_offset_candidates = [0_u16, SUB_ADDRESS_DDC_CI as u16, 0x50];
    let mut last_error = None;

    for chip in chip_candidates {
        for data_addr in data_addr_candidates {
            unsafe {
                let write_result = verify_io(IOAVServiceWriteI2C(
                    *service,
                    chip as _, // I2C chip address
                    data_addr as _,
                    // Skip the first byte, which is the I2C address, which this API does not need
                    request_data[1..].as_ptr() as _,
                    (request_data.len() - 1) as _,
                ));
            ...
            for read_offset in read_offset_candidates {
                out.fill(0);
                std::thread::sleep(response_delay);
                unsafe {
                    let read_result = verify_io(IOAVServiceReadI2C(
                        *service,
                        chip as _,
                        read_offset as _,
                        out.as_mut_ptr() as _,
    ...
    Err(last_error.unwrap_or(ServiceNotFound))
}
