...
pub(crate) fn execute<'a>(
    ...
    if request.replyTransactionType == kIOI2CNoTransactionType {
        ...
        let reply_len = (request.replyBytes as usize).min(out.len());
...
/// Get supported I2C / DDC transaction types for a specific interface.
unsafe fn get_supported_transaction_type(interface: &IoObject) -> Option<u32> {
    ...
    if ((1 << kIOI2CDDCciReplyTransactionType) & transaction_types) != 0 {
        Some(kIOI2CDDCciReplyTransactionType)
    } else if ((1 << kIOI2CSimpleTransactionType) & transaction_types) != 0 {
        Some(kIOI2CSimpleTransactionType)
    } else {
        None
    }
}
        ...
        if IOFBCopyI2CInterfaceForBus(service.into(), bus, &mut interface) == kIOReturnSuccess {
            ...
            if request.replyBytes > 0 {
                let transaction_type =
                    get_supported_transaction_type(&interface).unwrap_or(kIOI2CSimpleTransactionType);
                request.replyTransactionType = transaction_type;
                // Keep the DDC/CI sub-address for both simple and DDCci reply transactions.
                // Lunar/MonitorControl keep this at 0x51, and some adapters return malformed
                // payloads when simple transaction replies use sub-address 0.
